...
batch_size = 5 # Batch size for mini-batch gradient decent
...
def discount_rewards(r):
    ...
    # This is a constant discount rate
    discounted_r = np.zeros_like(r)
    running_add = 0
    for t in reversed(range(0, r.size)):
        # This is restart the reward discount
...
def policy_forward(x):
    """
    Foward each frame throught the network
    :param x: Each preprocessed frame
    :return: probability of taking action 2, and hidden state of layer 1
    """
    h = np.dot(model['W1'], x)
...
env = gym.make('Pong-v0',
    ...
    render_mode='human'               # None | human | rgb_array//Don't need to be modified
...
for i_episode in range(episodes):
    ...
    while episode_points > points:
        ...
        # Keep track of the amount of points score
        points += abs(reward)
    ...
    if i_episode % batch_size == 0:
        print('Updating models weights based on batch accumulated till episode {}'.format(i_episode))
